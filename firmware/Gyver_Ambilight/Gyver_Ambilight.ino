/*
   ============================================================================
   Adalight firmware — reference sketch bundled with the Adalight desktop app
   (https://github.com/xvin84/Adalight). Flash it onto your Arduino/ESP.

   ORIGINAL AUTHOR: AlexGyver — https://alexgyver.ru/arduino_ambilight/
   Source: https://github.com/AlexGyver/Arduino_Ambilight (Gyver_Ambilight v1.3)
   Included here WITH ATTRIBUTION and only lightly adapted. All credit for this
   firmware belongs to the original author; it keeps its original header below.

   Protocol: classic Adalight — the app sends the magic header "Ada" + count_hi +
   count_lo + (hi ^ lo ^ 0x55), then 3 bytes (R, G, B) per LED. This sketch reads
   exactly that, so it is fully compatible with the app out of the box.

   Match these to the app's Connection settings:
     - NUM_LEDS   -> total LED count (top + right + bottom + left)
     - serialRate -> app "Speed" (baud), e.g. 115200
     - channel order in the app -> RGB (the strip's physical GRB is applied by
       FastLED here via addLeds<..., GRB>, so the host must send true RGB)
