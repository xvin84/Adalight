...
| `api.register_frame_filter(id, fn, priority=50)` | Фильтр кадра — последняя обработка перед отправкой на ленту (см. ниже). |
...
настройки» — так редкие параметры (паспортные токи ленты, тонкая настройка
регулятора) не мешают обычному пользователю, но остаются под рукой. Скрыт
только вид: значение такого поля всё равно попадает в настройки.
...
Фильтр — последняя обработка перед отправкой на ленту:
`api.register_frame_filter(id, fn, priority=50)`. В `fn(colors)` приходит
готовый кадр `uint8` формы `(n_leds, 3)` — ровно те байты, что уйдут в ленту
(насыщенность, температура, гамма, яркость и вспышки уже применены). Вернуть
нужно изменённый массив или `None`, если менять нечего.

Фильтр работает во всех режимах — подсветка, лампа, цветомузыка и тесты ленты,
— а `priority` задаёт порядок в цепочке (меньше — раньше). Упавший фильтр
пропускается: сломанный мод не должен гасить подсветку. Кадр приходит из потока
движка, поэтому фильтр обязан быть быстрым (это горячий цикл) и не блокировать.

```python
class Halve:
    name = "halve"
    title = "Половинная яркость"

    def register(self, api):
        api.register_frame_filter(self.name, lambda colors: colors // 2)
...
Так устроен встроенный мод «Защита питания»
([`adalight/plugins/builtin/power_guard.py`](../adalight/plugins/builtin/power_guard.py)):
он оценивает ток ленты по сумме байтов кадра и приглушает её, пока та не
уложится в бюджет питания.

Второй пример — «Компенсация цвета стены»
([`examples/plugins/wall_tint.py`](../examples/plugins/wall_tint.py)): он делит
кадр на альбедо цветных обоев, чтобы отражение было ближе к задуманному цвету.
Там же видны два приёма для горячего цикла: множители сворачиваются в таблицу
256×3 при смене настроек (в фильтре остаётся одна выборка по индексу), а
состояние подменяется одной ссылкой — вместо блокировки на каждом кадре.

Кадр приходит уже после гамма-LUT ядра, а ШИМ WS2812B линеен по значению
байта, — то есть байт пропорционален излучаемому свету. Фильтру, который
работает с физикой света (ток, отражение, смешение), обратная гамма не нужна.
...
`power.status` (`{gain, current_ma, limit_ma, limited, floored}` — от «Защиты
питания»), `update.available` (`{version, url}`). Имена событий — свободные
строки, так что моды могут заводить свои.
...
Готовые рабочие плагины в [`examples/plugins/`](../examples/plugins) — копируйте
в папку плагинов, включайте и правьте под себя:

| Файл | Что показывает |
|---|---|
| [`break_reminder.py`](../examples/plugins/break_reminder.py) | Шаблон плагина: свой поток, вспышки, уведомления, форма настроек по схеме. |
| [`plasma_effect.py`](../examples/plugins/plasma_effect.py) | Свой эффект лампы через `register_lamp_effect`. |
| [`notification_logger.py`](../examples/plugins/notification_logger.py) | Подписка на события шины. |
| [`wall_tint.py`](../examples/plugins/wall_tint.py) | Фильтр кадра: компенсация цвета обоев за монитором (таблица вместо счёта на каждом кадре). |
