...
def _load_from_module(module) -> LoadedPlugin:
...
def discover() -> list[LoadedPlugin]:
    ...
    for module_name in BUILTIN_MODULES:
        try:
            found.append(_load_from_module(importlib.import_module(module_name)))
    ...
    if user_dir.is_dir():
        for path in sorted(user_dir.glob("*.py")):
            try:
                ...
                found.append(_load_from_module(module))
            except Exception as e:  # noqa: BLE001
                found.append(LoadedPlugin(None, path.stem, path.name, "", error=str(e)))
    return found
